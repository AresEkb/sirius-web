...
git cherry-pick den/enh/graphql-subscription-exception
echo Success

# Publish frontend

# find -type f -regextype egrep -iregex '.*\.(json|ts|tsx)' -not -path '*/node_modules/*' -not -path '*/dist/*' -not -path '*/backend/*' -exec sed -i 's/@eclipse-sirius/@metamodel/' {} \;
# npm ci
